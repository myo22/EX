...
public class MathBeanTest {
    public static void main(String[] args){ //static이 붙은 메소드는 해당 메소드를 가진 클래스가 인스턴스가 되지 않아도 사용할 수 있으므로 main 메소드가 실행가능한 것이다.
        MathBean math = new MathBean(); // MathBean 인스턴스는 heap 메모리에 올라간다. math가 리코몬과 비슷한 개념이라고 봐도 괜찮다 ex) TV 리모콘 = new TV() 리모콘을 누르면 TV전원이 켜지는 것처럼 math를 키면 MathBean이 켜지는 것이다.
        math.printClassName();
